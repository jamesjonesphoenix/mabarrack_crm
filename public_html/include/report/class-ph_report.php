    ...
    function setup_activity_summary( $shifts = array() ) {
        ...
        foreach ( $shifts as $key => $shift ) {
            if ( empty( $activities_summary[ $shift[ 'activity' ] ] ) ) {
                $activity_id = $activities->get_activity_id( $shift[ 'activity' ] );
                $activities_summary[ $shift[ 'activity' ] ] = array(
                    'activity_ID' => $activity_id ? $activity_id : 'N/A',
    ...
    function setup_shifts() {
        ...
        if ( !empty( $shifts ) ) {
            ...
            foreach ( $shifts as &$shift ) {
                ...
                $shift[ 'activity' ] = $this->activities->get_activity_name( $shift[ 'activity' ] );
