...
if ( function_exists( 'get_rows' ) ) {
    ...
    if ( $unfinished_shifts !== FALSE ) { //there are unfinished shifts from the day
        ...
        $ph_cron_log->add_log( 'Found ' . $found_message );
        foreach ( $unfinished_shifts as $unfinished_shift ) {
            ...
            if ( $result !== TRUE ) {
                $ph_cron_log->add_log( 'Failed to update shift - ' . $result );
                exit();
            } else
                $ph_cron_log->add_log( 'Successfully clocked off shift with ID of ' . $unfinished_shift[ 'ID' ] . '.' );
        }
    } else
        $ph_cron_log->add_log( 'No unfinished shifts found today.' );
} else
    $ph_cron_log->add_log( 'get_rows() doesn\'t exist. Probably means we failed to include the functions library.' );
$ph_cron_log->add_log( 'Finished' );
